...
def CalcMinMax(
    ...
    '''
    vmin = min(reach_df[reach_df[var_reach] > 0][var_reach].min(
    ), catch_df[catch_df[var_catch] > 0][var_catch].min())
...
def PlotMaps(
    ...
    if diff == False:
        ...
        mid_catch = gdf_catch[var_catch.split(
            '_')[0] + '_loadrate'].quantile(0.85)
        max_reach = gdf_reach[var_reach.split('_')[0] + '_conc'].quantile(0.99)
        max_catch = gdf_catch[var_catch.split(
            '_')[0] + '_loadrate'].quantile(0.99)
    # Display min, mid, max
    print(
    ...
    if include_reach == True:
        if zoom == False:
            major_streams = gdf_reach[gdf_reach['streamorder'] >=
                                      5].loc[:, ('streamorder', gdf_reach.geometry.name)]
            rch = major_streams.plot(
                linewidth=major_streams['streamorder'] % 4, ax=ax2, color='cornflowerblue', zorder=10)
        else:
            major_streams = gdf_reach[gdf_reach['streamorder'] >=
                                      4].loc[:, ('streamorder', gdf_reach.geometry.name)]
            rch = major_streams.plot(linewidth=(
                major_streams['streamorder'] + 1) % 4, ax=ax2, color='cornflowerblue', zorder=10)
    ...
    if cl != None:
        ...
        # plot cluster
        cl_reach = cluster_gdf[cluster_gdf.index == cl].plot(
            lw=1, ax=ax1, facecolor="none", edgecolor="black", zorder=10)
        cl_catch = cluster_gdf[cluster_gdf.index == cl].plot(
            lw=1, ax=ax2, facecolor="none", edgecolor="black")
    ...
    if fa == True:
        ...
        fa_reach = fas_in_cluster.plot(
            lw=0.7, ax=ax1, facecolor="none", edgecolor="grey", zorder=10)
        fa_catch = fas_in_cluster.plot(
            lw=0.7, ax=ax2, facecolor="none", edgecolor="grey")
    ...
    if zoom == True:
        if cl == None:
            ...
            lon_max, lon_min, lat_max, lat_min, area, h_v = LatLonExtent(
                cl, cluster_gdf)
            for ax in [ax1, ax2]:
    ...
    cax = fig.add_axes([0.95, 0.18, 0.02, 0.64])
    ...
    # add colorbar - reach
    # adjusts the position of the color bar: right position, bottom, width, top
    cax2 = fig.add_axes([0.48, 0.18, 0.02, 0.64])
    ...
    for ax in [ax1, ax2]:
        if zoom == False:
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                            crs=dp_reach.crs.to_string(), zoom=7, interpolation='sinc')
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.PositronOnlyLabels,
                            crs=dp_reach.crs.to_string(), zoom=7, zorder=2, interpolation='sinc')
        else:
            # change zoom of basemap based on coverage area
            if area < 7:
                ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                                crs=dp_reach.crs.to_string(), zoom=10, interpolation='sinc')
                ctx.add_basemap(ax, source=ctx.providers.CartoDB.PositronOnlyLabels,
                                crs=dp_reach.crs.to_string(), zoom=10, zorder=2, interpolation='sinc')
            else:
                ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                                crs=dp_reach.crs.to_string(), zoom=9, interpolation='sinc')
                ctx.add_basemap(ax, source=ctx.providers.CartoDB.PositronOnlyLabels,
                                crs=dp_reach.crs.to_string(), zoom=9, zorder=2, interpolation='sinc')
...
def Extent_Map(
    ...
    cl_catch = cluster_gdf[cluster_gdf.index == cl].plot(
        lw=1, ax=ax, facecolor="none", edgecolor="black")

    gdf_reach = base_reach_gdf
    major_streams = gdf_reach[gdf_reach['streamorder'] >=
                              5].loc[:, ('streamorder', gdf_reach.geometry.name)]
    rch = major_streams.plot(
        linewidth=major_streams['streamorder'] % 4, ax=ax, color='cornflowerblue', zorder=10)

    fig.set_size_inches(8, 8)
    FormatAxes(ax)
    ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                    crs=gdf_reach.crs.to_string(), zoom=7)
...
def PlotZoom(gdf_reach, gdf_catch, var_reach, var_catch, targ_reach, targ_catch, cl=None):
    ...
    for ax in [ax1, ax2, ax3, ax4]:
        cluster_gdf[cluster_gdf.index == cl].plot(
            lw=1, ax=ax, facecolor="none", edgecolor="black", zorder=10)
        fas_in_cluster.plot(lw=0.7, ax=ax, facecolor="none",
    ...
    if h_v == "vertical":
        for ax in [ax1, ax2]:
            FormatAxes(ax, bounds=[lon_min, lon_max,
                       lat_min + (lat_max - lat_min) / 2, lat_max])
        for ax in [ax3, ax4]:
            FormatAxes(ax, bounds=[lon_min, lon_max,
                       lat_min, lat_min + (lat_max - lat_min) / 2])
        plot_order = [ax1, ax2, ax3, ax4]
    elif h_v == 'horizontal':
        for ax in [ax1, ax3]:
            FormatAxes(ax, bounds=[lon_min, lon_min -
                       (lon_min - lon_max) / 2, lat_min, lat_max])
        for ax in [ax2, ax4]:
            FormatAxes(
                ax, bounds=[lon_min - (lon_min - lon_max) / 2, lon_max, lat_min, lat_max])
    ...
    if h_v == "vertical":
        ...
        ax1.set_title("%s for Reaches \n %s %s Cluster" %
                      (var_reach, "Eastern", cl))
        ax2.set_title("%s for Reaches \n %s %s Cluster" %
                      (var_reach, "Western", cl))
        ax3.set_title("%s for Catchments \n %s %s Cluster" %
                      (var_catch, "Eastern", cl))
        ax4.set_title("%s for Catchments \n %s %s Cluster" %
                      (var_catch, "Western", cl))

    for ax in [ax1, ax2, ax3, ax4]:
        if area < 7:
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                            crs=gdf_reach.crs.to_string(), zoom=10, interpolation='sinc')
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.PositronOnlyLabels,
                            crs=gdf_reach.crs.to_string(), zoom=10, zorder=2, interpolation='sinc')
        else:
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.Positron,
                            crs=gdf_reach.crs.to_string(), zoom=9, interpolation='sinc')
            ctx.add_basemap(ax, source=ctx.providers.CartoDB.PositronOnlyLabels,
                            crs=gdf_reach.crs.to_string(), zoom=9, zorder=2, interpolation='sinc')
...
def add_colorbar(
    ...
    # adjusts the position of the color bar: right position, bottom, width, top
    cax2 = fig.add_axes([0.89, 0.107, 0.04, 0.733])
    sm2 = plt.cm.ScalarMappable(cmap=colormap,
                                norm=lognorm_geom)
    cbr2 = fig.colorbar(sm2, cax=cax2,)
    cbr2.ax.minorticks_off()
...
def PlotMaps_FA_single_pane(
    ...
    if comid_type == 'catchment':
        ...
        if include_reach == True:
            major_streams = streamorder_gdf[streamorder_gdf['streamorder']
                                            >= 3].loc[:, ('streamorder', streamorder_gdf.geometry.name)]
    ...
    if cl == None:
        ...
        lon_max, lon_min, lat_max, lat_min, area, h_v = LatLonExtent_FA(
            list(fas_in_cluster.index), focusarea_gdf)
...
def plot_remaining_work(
    ...
    # Add title
    plt.title(
        f'Percent of Protected Land in HUC 12s where \n{threshold}% Natural Land Threshold is Possible')
...
def plot_remaining_work_dif(
    ...
    # Only include areas where meeting the threshold of protected natural land is feasible
    feasible_gdf = gdf.loc[(gdf['perc_natural'] >= plot_range[0]) & (
        gdf['perc_natural'] < plot_range[1])]
    ...
    # Add title
    plt.title(
        f'Percent of Protection Remaining to Meet {threshold}% Threshold \n(in HUC 12s where the threshold is possible)')
...
def plot_protec_nat(
    ...
    if nat_protect_type not in nat_protect_types:
        raise ValueError(
            'Invalid nat_protec_type. Expected one of: %s' % nat_protect_types)
